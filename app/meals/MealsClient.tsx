...
async function loadMealsAction(
  _prev: ViewState,
  ...
  try {
    ...
    return { status: "ready", meals: json.meals };
  } catch {
    return { status: "error", message: "Failed to load meals." };
  }
}
...
export default function MealsClient() {
  const [view, runLoad] = useActionState(loadMealsAction, { status: "boot" } as ViewState);
        ...
        {view.status === "boot" ? (
          ...
          <>
            ...
            <div className="mt-10 rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50 to-emerald-100 px-6 py-8 text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Consistency beats perfection. Follow this plan for 10–14 days, then regenerate with better accuracy.
              </p>
            </div>
